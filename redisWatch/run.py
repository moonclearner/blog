    ...
    def run(self):
        while 1:
            try:
                ...
                self.nowtime = datetime.datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S %Z")
                # status
                ...
                if len(self.table) == 0:
                    ...
                    self.commands_per_seconds = (
                        redis_info['total_commands_processed'] -
                        self.last_total_commands_processed) / INFO_INTERVAL
                    self.table_row.append(self.commands_per_seconds)
                    self.table_row.append(
                        (redis_info['expired_keys'] - self.last_expired_keys) /
                        INFO_INTERVAL)
                    self.table_row.append(
                        (redis_info['evicted_keys'] - self.last_evicted_keys) /
                        INFO_INTERVAL)
                    self.table_row.append(
                        (redis_info['keyspace_hits'] - self.last_keyspace_hits)
                        / INFO_INTERVAL)
                    self.table_row.append(
                        (redis_info['keyspace_misses'] -
                         self.last_keyspace_misses) / INFO_INTERVAL)

                self.last_total_commands_processed = redis_info[
                ...
                # commands highchart
                self.commands_chart.append({
                    'x': self.nowtime,
                    'y': self.commands_per_seconds
                })
                # cpu usage (system and user)
                self.cpu_chart.append({
                    'x': self.nowtime,
                    'y_s': self.used_cpu_sys,
                    'y_u': self.used_cpu_user
                })
                # memory usage
                self.mem_chart.append({
                    'x': self.nowtime,
                    'y_mem': self.mem,
                    'y_rss': self.mem_rss
                })
...
def client_command(message):
    ...
    cmd_threads = [
        t for t in all_thread if '{0}:{1}'.format(t.host, t.port) == r_server
    ]
    # print(cmd_threads)
    if cmd_threads:
